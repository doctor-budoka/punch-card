...
mod tests {
    ...
    fn test_returns_none_for_max_size_0(vec_size: usize) {
        let test_list = (0..vec_size)
            .into_iter()
            .map(|_x| generate_random_string(10))
            .collect();
    ...
    fn test_under_the_max(num_items_to_test: usize, test_max_size: Option<usize>) {
        let test_list = (0..num_items_to_test)
            .into_iter()
            .map(|_x| generate_random_string(10))
            .collect();
    ...
    fn test_over_the_max(num_items_to_test: usize, test_max_size: Option<usize>) {
        ...
        let expected: String =
            Vec::from_iter(test_list.iter().take(max_size).map(|x| x.to_owned())).join(", ")
                + ", ...";
        assert_eq!(out, expected);
    }
}
